...
export async function POST(request: NextRequest) {
  return handleGatewayRequest(request, "openai_responses", {
    parseBody: async (req) => (await req.json()) as OpenAIResponsesRequest,
    extractApiKey: (req) => {
      return extractBearerToken(req.headers.get("Authorization"));
    },
    toUnified: openaiResponsesToUnified,
    toResponse: unifiedToOpenaiResponsesResponse,
    estimateInputTokensFallback: (body) => estimateTokens(JSON.stringify(body.input)),
    errors: {
